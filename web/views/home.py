...
def index(request, *args, **kwargs):
    ...
    all={}
    art_count= models.Article.objects.all().count()
    all['article'] =art_count
    commet_count = models.Comment.objects.all().count()
    all['comment'] = commet_count
    obj =models.Article.category_choices

    for item in obj:
        a= models.Article.objects.filter(category_id= item[0]).count()
        all[item[1]]= a
...
def home(request, site):
    ...
    category_list = models.Article.category_choices
    #date_format(create_time,"%Y-%m")
    date_list = models.Article.objects.raw(
        'select nid, count(nid) as num,strftime("%Y-%m",create_time) as ctime from lee_article group by strftime("%Y-%m",create_time)')
...
def detail(request, site, nid):
    ...
    a= models.Article.objects.raw(
        'select nid,category_id,count(nid) as num from lee_article group by category_id')
    print ('3333')
    print(a)
    for item in a:
        print(item.nid,item.category_id,item.num)
